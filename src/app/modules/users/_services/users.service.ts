    ...
    getName(params = 'name',  filter : string = ''): Observable<BaseResponse<User[]>> {
    const url = `${this.baseUrl}?${params}=${filter}`;
    return this.http.get<BaseResponse<User[]>>(url);
    }
  
    getEmail(params = 'email',  filter : string = ''): Observable<BaseResponse<User[]>> {
    const url = `${this.baseUrl}?${params}=${filter}`;
    return this.http.get<BaseResponse<User[]>>(url);
    }
